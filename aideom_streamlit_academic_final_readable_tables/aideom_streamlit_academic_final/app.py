...
st.markdown(
    ...
    /* Extra readable tables and Plotly containers */
    div[data-testid="stPlotlyChart"] {background:#ffffff !important; border:1px solid #d8d0bd !important; border-radius:20px !important; padding:.75rem !important; box-shadow:0 16px 34px -24px rgba(26,40,73,.35) !important;}
    div[data-testid="stPlotlyChart"] * {color:#10243f !important;}
    .stDataFrame, div[data-testid="stDataFrame"], div[data-testid="stDataEditor"] {background:#ffffff !important; border:1px solid #d8d0bd !important; border-radius:18px !important; padding:.35rem !important;}
    .stDataFrame div, div[data-testid="stDataFrame"] div, div[data-testid="stDataEditor"] div {color:#07172e !important;}
    div[data-testid="stDataFrame"] [role="columnheader"], div[data-testid="stDataEditor"] [role="columnheader"] {background:#e9f7f4 !important; color:#07172e !important; font-weight:800 !important;}
...
# ===== Plotly/readability helpers =====
PLOTLY_CONFIG = {"displayModeBar": True, "responsive": True}

def polish_fig(fig, height=430):
    """Force readable light Plotly charts even when browser/system uses dark mode."""
    fig.update_layout(
        template="plotly_white",
        height=height,
        paper_bgcolor="#ffffff",
        plot_bgcolor="#ffffff",
        font=dict(color="#10243f", family="Inter, Arial, sans-serif", size=13),
        title=dict(font=dict(color="#1a2849", size=18), x=0.02, xanchor="left"),
        legend=dict(
            bgcolor="rgba(255,255,255,.92)",
            bordercolor="#d8d0bd",
            borderwidth=1,
            font=dict(color="#10243f"),
        ),
        margin=dict(l=40, r=35, t=65, b=45),
        hoverlabel=dict(bgcolor="#ffffff", font_color="#10243f", bordercolor="#d8d0bd"),
    )
    fig.update_xaxes(
        showgrid=True, gridcolor="#e7edf3", zerolinecolor="#d8d0bd",
        linecolor="#9ca3af", tickfont=dict(color="#10243f"), title_font=dict(color="#10243f")
    )
    fig.update_yaxes(
        showgrid=True, gridcolor="#e7edf3", zerolinecolor="#d8d0bd",
        linecolor="#9ca3af", tickfont=dict(color="#10243f"), title_font=dict(color="#10243f")
...
def show_fig(fig, height=430):
    st.plotly_chart(polish_fig(fig, height=height), use_container_width=True, config=PLOTLY_CONFIG)

def readable_df(df, use_container_width=True, hide_index=True, **kwargs):
    """Use styled dataframe/table so text contrast is consistent."""
    try:
        styler = df.style.set_properties(**{
            ...
            {"selector":"th", "props":[("background-color", "#e9f7f4"), ("color", "#07172e"), ("font-weight", "800"), ("border", "1px solid #d8d0bd")]},
            {"selector":"td", "props":[("border", "1px solid #eee4d1")]},
            {"selector":"caption", "props":[("color", "#53657c")]},
        ])
        st.dataframe(styler, use_container_width=use_container_width, hide_index=hide_index, **kwargs)
    except Exception:
        st.dataframe(df, use_container_width=use_container_width, hide_index=hide_index, **kwargs)
...
def plot_bar(df, x, y, title, color=None):
    ...
    fig.update_traces(textfont_color="#10243f", marker_line_color="#ffffff", marker_line_width=1)
    fig.update_layout(legend_title_text="")
    show_fig(fig, height=430)
...
def bai1():
    ...
    with tabs[1]:
        ...
        readable_df(contrib, use_container_width=True)
...
def bai2():
    ...
    with tabs[0]:
        ...
        readable_df(items, use_container_width=True)
    with tabs[1]:
        ...
        readable_df(out, use_container_width=True)
...
def bai3():
    ...
    with tabs[1]:
        ...
        readable_df(df[["sector_name_vi", "Priority", "ai_readiness_0_100", "export_billion_USD", "gdp_share_2024_pct", "labor_million"]], use_container_width=True)
...
def bai4():
    ...
    with tabs[0]:
        ...
        readable_df(temp, use_container_width=True)
    with tabs[1]:
        alloc = regions.copy()
        ...
        readable_df(alloc[["region_name_vi", "digital_index_0_100", "Ngân sách cần tối thiểu", "D sau đầu tư"]], use_container_width=True)
...
def bai5():
    ...
    with tabs[0]:
        ...
        readable_df(projects, use_container_width=True)
    with tabs[1]:
        best = None
        ...
        if best:
            ...
            readable_df(chosen, use_container_width=True)
...
def bai6():
    ...
    with tabs[1]:
        ...
        readable_df(df[["region_name_vi", "TOPSIS"]+cols], use_container_width=True)
...
def bai7():
    ...
    with tabs[1]:
        ...
        readable_df(obj.sort_values("Điểm thỏa hiệp", ascending=False).head(10), use_container_width=True)
...
def bai8():
    ...
    with tabs[1]:
        ...
        readable_df(df, use_container_width=True)
...
def bai9():
    ...
    with tabs[1]:
        ...
        readable_df(df[["sector_name_vi", "Displaced", "Retrained/Upgraded", "NewJob", "NetJob"]], use_container_width=True)
...
def bai11():
    ...
    with tabs[2]:
        ...
        readable_df(pol, use_container_width=True)
...
def bai12():
    ...
    with tabs[0]:
        ...
        readable_df(design, use_container_width=True)
    ...
    with tabs[2]:
        readable_df(scenarios, use_container_width=True)
