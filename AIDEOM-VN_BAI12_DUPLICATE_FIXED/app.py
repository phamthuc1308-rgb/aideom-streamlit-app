...
def page1() -> None:
    ...
    if st.button("🚀 Chạy mô hình Bài 1"):
        ...
        st.plotly_chart(b1.fig_tfp(df), use_container_width=True, key='b1_tfp')
        st.plotly_chart(b1.fig_actual_vs_pred(df), use_container_width=True, key='b1_actual_pred')
        st.plotly_chart(b1.fig_contrib(contrib), use_container_width=True, key='b1_contrib')
...
def page2() -> None:
    ...
    if st.button("🚀 Chạy LP Bài 2"):
        ...
        st.plotly_chart(b2.fig_allocation(df), use_container_width=True, key='b2_alloc')
        st.dataframe(df, use_container_width=True, hide_index=True)
        if not shadow.empty:
            st.dataframe(shadow, use_container_width=True, hide_index=True)
        st.plotly_chart(b2.fig_sensitivity(b2.sensitivity_curve(tuple(floors))), use_container_width=True, key='b2_sens')
...
def page3() -> None:
    ...
    if st.button("🚀 Tính Priority"):
        ...
        st.plotly_chart(b3.fig_rank(rank), use_container_width=True, key='b3_rank')
        st.dataframe(rank, use_container_width=True, hide_index=True)
        with st.expander("Xem ma trận chuẩn hóa"):
            st.dataframe(norm, use_container_width=True)
        st.plotly_chart(b3.fig_heatmap(b3.sensitivity_ai()), use_container_width=True, key='b3_heatmap')
...
def page4() -> None:
    ...
    if st.button("🚀 Chạy LP vùng"):
        ...
        if status != "Optimal":
            ...
            st.plotly_chart(b4.fig_heatmap(df), use_container_width=True, key='b4_heatmap')
            st.plotly_chart(b4.fig_region_totals(df), use_container_width=True, key='b4_region_totals')
            st.dataframe(df, use_container_width=True, hide_index=True)
...
def page5() -> None:
    ...
    if st.button("🚀 Chạy MIP"):
        ...
        st.plotly_chart(b5.fig_roi(df), use_container_width=True, key='b5_roi')
        st.dataframe(df, use_container_width=True, hide_index=True)
...
def page6() -> None:
    ...
    if st.button("🚀 Tính TOPSIS"):
        ...
        st.plotly_chart(b6.fig_scores(comp), use_container_width=True, key='b6_scores')
        st.dataframe(rank, use_container_width=True, hide_index=True)
...
def page7() -> None:
    ...
    if st.button("🚀 Chạy Pareto"):
        ...
        st.plotly_chart(b7.fig_3d(scored), use_container_width=True, key='b7_3d')
        st.plotly_chart(b7.fig_parallel(scored.head(100)), use_container_width=True, key='b7_parallel')
...
def page8() -> None:
    ...
    if st.button("🚀 Tối ưu động"):
        ...
        st.plotly_chart(b8.fig_trajectories(df), use_container_width=True, key='b8_traj')
...
def page9() -> None:
    ...
    if st.button("🚀 Chạy mô phỏng lao động"):
        ...
        st.plotly_chart(b9.fig_alloc(df), use_container_width=True, key='b9_alloc')
        st.plotly_chart(b9.fig_jobs(df), use_container_width=True, key='b9_jobs')
        st.dataframe(df, use_container_width=True, hide_index=True)
...
def page10() -> None:
    ...
    if st.button("🚀 Chạy stochastic programming"):
        ...
        st.plotly_chart(b10.fig_x(xdf), use_container_width=True, key='b10_x')
        st.plotly_chart(b10.fig_metrics(metrics), use_container_width=True, key='b10_metrics')
...
def page11() -> None:
    ...
    if st.button("🚀 Train Q-learning"):
        ...
        st.plotly_chart(b11.fig_curve(curve), use_container_width=True, key='b11_curve')
...
def page12() -> None:
    """Render Bài 12 in the compact dashboard layout requested by the user.

    This screen intentionally follows the reference screenshot: large plain title,
    small chips under the title, short description lines, compact tabs, then
    M1/M2 action blocks and the system-design table on the overview tab.
    """
    html_block(
        """
        <div class="b12-wrap">
          <h1 class="b12-title">🇻🇳 Bài 12 — AIDEOM-VN Dashboard tích hợp</h1>
          <div class="b12-chip-row">
            <span class="b12-pill hot">ĐỒ ÁN TÍCH HỢP</span>
            <span class="b12-pill">6 module M1–M6</span>
            <span class="b12-pill">5 kịch bản chính sách</span>
          </div>
          <p class="b12-lead">Mô hình AIDEOM-VN tích hợp <b>6 module</b>: Dự báo (M1) → Sẵn sàng số (M2) → Phân bổ (M3) → Lao động (M4) → Đánh giá rủi ro (M5) → Dashboard (M6).</p>
          <p class="b12-lead">So sánh <b>5 kịch bản chính sách</b> trong Mục 15 của bài báo nguồn.</p>
        </div>
        """
    )
    ...
    with tab1:
        html_block('<div class="b12-section"><h2>M1 — Dự báo kinh tế (Cobb-Douglas)</h2></div>')
        if st.button("Chạy M1", key="b12_run_m1"):
            st.success("M1 đã chạy: cập nhật GDP, TFP và dự báo 2030.")
            gdp_last = float(kpi.sort_values("GDP_index", ascending=False).iloc[0]["GDP_index"])
            metric_row([
                ("GDP_index cao nhất", f"{gdp_last:,.2f}", None),
                ("Số kịch bản", "5", None),
                ("Năm kết thúc", "2030", None),
            ])
            st.plotly_chart(b12.fig_gdp(df), use_container_width=True, key='b12_gdp_overview')

        html_block('<div class="b12-divider"></div>')
        html_block('<div class="b12-section"><h2>M2 — Đánh giá sẵn sàng số (TOPSIS)</h2></div>')
        if st.button("Chạy M2", key="b12_run_m2"):
            st.success("M2 đã chạy: tổng hợp Digital Index và AI Readiness cho dashboard.")
            st.dataframe(kpi[["Kịch bản", "D", "AI", "H"]], use_container_width=True, hide_index=True)

        html_block('<div class="b12-divider"></div>')
        html_block('<div class="b12-section"><h2>📋 Tóm tắt thiết kế hệ thống</h2></div>')
        system_df = pd.DataFrame([
            ["M1", "Dự báo kinh tế", "Macro 2020–2025", "GDP, TFP 2030", "Cobb-Douglas (Bài 1)"],
            ["M2", "Sẵn sàng số", "Sectors, Regions", "Digital + AI Index", "TOPSIS (Bài 6)"],
            ["M3", "Tối ưu phân bổ", "Budget, β-matrix", "Phân bổ ngành-vùng", "LP (Bài 4) + Dynamic (Bài 8)"],
            ["M4", "Lao động", "x_AI, x_H", "NetJob từng ngành", "LP (Bài 9)"],
            ["M5", "Rủi ro", "Risk params", "Cyber, Env, Dependency", "NSGA-II (Bài 7) + SP (Bài 10)"],
            ["M6", "Dashboard", "Outputs M1–M5", "Trực quan kịch bản", "Streamlit + Plotly"],
        ], columns=["Module", "Tên", "Đầu vào", "Đầu ra", "Kỹ thuật"])
        st.dataframe(system_df, use_container_width=False, hide_index=True, height=260)

    with tab2:
        html_block('<div class="b12-section"><h2>M3 — Phân bổ ngân sách ngành-vùng</h2></div>')
        st.info("Tab này gom kết quả phân bổ từ Bài 4 và quỹ đạo động từ Bài 8 để phục vụ đồ án tích hợp.")
        alloc = pd.DataFrame({
            "Hạng mục": ["K", "D", "AI", "H"],
            ...
            "S5 Tối ưu cân bằng": [35, 28, 20, 17],
        })
        st.dataframe(alloc, use_container_width=True, hide_index=True)
        st.plotly_chart(b12.fig_gdp(df), use_container_width=True, key='b12_gdp_alloc')
        analysis("M3 không chỉ tối đa hóa GDP gain mà còn phải giữ tính công bằng vùng và khả năng triển khai. S5 được dùng làm phương án cân bằng để tránh cực đoan vào một hạng mục duy nhất.")

    with tab3:
        html_block('<div class="b12-section"><h2>📋 5 kịch bản chính sách đến 2030</h2></div>')
        c1, c2 = st.columns([1.15, 1])
        with c1:
            st.plotly_chart(b12.fig_gdp(df), use_container_width=True, key='b12_gdp_scenarios')
        with c2:
            st.plotly_chart(b12.fig_radar(kpi), use_container_width=True, key='b12_radar_scenarios')
        st.dataframe(kpi[["Kịch bản", "GDP_index", "D", "AI", "H", "Risk_control"]], use_container_width=True, hide_index=True)
        analysis("S1 có vai trò làm kịch bản nền, S2 đẩy nhanh số hóa, S3 ưu tiên AI, S4 ưu tiên bao trùm và S5 là nghiệm cân bằng từ pipeline tích hợp. Bảng KPI giúp so sánh rõ đánh đổi giữa tăng trưởng, năng lực số, nhân lực và kiểm soát rủi ro.")

    with tab4:
        html_block('<div class="b12-section"><h2>⚠️ Cảnh báo rủi ro M4–M5</h2></div>')
        risk_df = kpi[["Kịch bản", "AI", "H", "Risk_control"]].copy()
        risk_df["Cảnh báo"] = np.where(risk_df["Risk_control"] < risk_df["Risk_control"].median(), "Cần tăng H / governance", "Ổn định")
        st.dataframe(risk_df, use_container_width=True, hide_index=True)
        st.plotly_chart(b12.fig_radar(kpi), use_container_width=True, key='b12_radar_risk')
        analysis("Khi AI tăng nhanh hơn vốn nhân lực H, rủi ro dữ liệu và rủi ro triển khai tăng. Vì vậy dashboard nên luôn hiển thị cảnh báo đi kèm khuyến nghị tăng đào tạo, governance dữ liệu và kiểm thử chính sách theo giai đoạn.")
