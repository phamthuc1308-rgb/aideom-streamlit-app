...
st.markdown(
    """
    <style>
    .stApp {background: linear-gradient(135deg,#eefaf6 0%,#f7fbff 46%,#fff7de 100%); color:#08152e;}
    section[data-testid="stSidebar"] {background:#0e1730; color:#fff;}
    section[data-testid="stSidebar"] * {color:#f8fbff !important;}
    .main-title {font-size:2.4rem; font-weight:900; letter-spacing:-.04em; margin-bottom:.2rem;}
    .subtitle {font-size:1.05rem; color:#58708a; margin-bottom:1.2rem;}
    .badge {display:inline-flex; align-items:center; gap:.35rem; padding:.35rem .72rem; border-radius:999px; background:linear-gradient(135deg,#e34d8d,#7636b8); color:white; font-weight:800; font-size:.78rem; margin-right:.4rem;}
    .soft-card {background:rgba(255,255,255,.84); border:1px solid rgba(25,130,118,.18); border-radius:22px; padding:1.15rem 1.2rem; box-shadow:0 14px 40px rgba(20,72,95,.10); margin-bottom:1rem;}
    .metric-card {background:white; border:1px solid #d9eee9; border-radius:18px; padding:1rem; box-shadow:0 8px 24px rgba(20,72,95,.08);}
    .formula-box {background:#0e1730; color:#f9fbff; border-radius:18px; padding:1rem 1.2rem; margin:.8rem 0; border-left:6px solid #12b8aa;}
    .note {background:#fff7db; border-left:5px solid #d89b2b; padding:1rem; border-radius:12px; color:#33240b;}
    .success-box {background:#ddfbef; border-left:5px solid #13916f; padding:1rem; border-radius:12px; color:#073b30;}
    .warn-box {background:#ffe8e8; border-left:5px solid #d94848; padding:1rem; border-radius:12px; color:#4c1010;}
    div[data-testid="stMetric"] {background:white; border:1px solid #d9eee9; padding:1rem; border-radius:18px; box-shadow:0 8px 24px rgba(20,72,95,.08);}
...
def header(title, subtitle, level="AIDEOM-VN"):
    ...
    st.markdown(f"<span class='badge'>{level}</span><span class='badge'>Streamlit tương tác</span>", unsafe_allow_html=True)
...
if pages[choice] is None:
    header("AIDEOM-VN — Dashboard Streamlit bổ sung", "Bản này đưa các phần trong HTML như kịch bản tăng trưởng, kết quả, phân tích và tab tương tác vào Streamlit.", "TRANG GIỚI THIỆU")
    card("<b>Mục tiêu:</b> biến báo cáo tĩnh thành dashboard học thuật có thể điều chỉnh tham số, quan sát kết quả và đọc nhận xét chính sách ngay trong từng bài.")
